...
namespace Emedchina.TradeAssistant.Client.DAL.Map.Hospital
{
    /// <summary>
    /// 医院编码对照DAL层
    /// </summary>
    class HosptailIDCompareDAO : SqlDAOBase
    {
        ...
        public string GetListSql()
        {
            ...
            sql.Append(" AS spell_abbr,b.name_wb AS name_wb,");
            sql.Append("case when a.Process_Flag='0' or a.Process_Flag is null  then '未处理' when a.Process_Flag='1' then '已处理'end as Process_Flag,");
            ...
            sql.Append(" case when a.IsMap='0' or a.IsMap is null then '未匹配'  else '已匹配' end  AS IsMap,a.IsMap as IsMapFlag,");
            sql.Append(" a.Process_Flag as pfFlag from gpo_corp_map_view  a ");
        ...
        public string GetCompareListSql()
        {
            ...
            sb.Append(" ( select * from gpo_reg_buyer) as buyer left join ( select b.org_id, case when count(b.id) ");
            sb.Append(" is null then '0' else count(b.id) end as MapSum from GPO_CORP_MAP_VIEW b group by b.org_id ) ");
        ...
        public string GetEmedCorpListDsSQL()
        {
            ...
            sb.Append(" ( select b.org_id, case when count(b.id) is null then '0' else count(b.id) end as MapSum");
            sb.Append(" from GPO_CORP_MAP_VIEW b group by b.org_id ) as hisCorp on buyer.id = hisCorp.org_id ");
        ...
        {
            return ("select * from GPO_CORP_MAP_VIEW where org_id = '" + inOrgId + "'");
        }
